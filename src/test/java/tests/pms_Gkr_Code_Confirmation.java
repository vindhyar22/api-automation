...
public class pms_Gkr_Code_Confirmation {
	...
	public void getKeyAndVerifyCodeDelivery() throws Throwable {
		...
		for (int i = 0; i < Key_IDs.size(); i++) {
			...
			System.out.println("Processing Key_ID: " + Key_IDs.get(i));
			...
			for (int j = 0; j < deviceIds.size(); j++) {
				// System.out.println(" deviceId -----" + deviceIds);
				deviceId = response3.jsonPath().getString("data[" + j + "].id");
				System.out.println(" deviceId  -----" + deviceIds.get(j));
				codedelievered = response3.jsonPath().getString("data[" + j + "].isDelivered");

				if ("true".equals(codedelievered)) {
				...
				System.out.println("After response4");
				if ("true".equals(codedelievered)) {
